...
博客使用简洁的 Markdown 格式，文章列表会**自动生成**，只需要按照约定的格式编写即可。
...
发布时间：YYYY-MM-DD · 标签：#标签1 #标签2 #标签3
...
| 位置 | 内容 | 说明 | 是否必填 |
|------|------|------|----------|
| 第一行 | `# 文章标题` | 文章标题，`# ` 后面接标题文字 | ✅ 必填 |
| 第三行 | `发布时间：YYYY-MM-DD · 标签：#标签1 #标签2` | 发布日期 + 标签，日期格式必须是 `YYYY-MM-DD` | 日期必填，标签可选 |
| 空一行后 | `---` | 分隔线，分隔头部信息和正文 | ✅ 必填 |

> 💡 **自动生成**：每次构建时，脚本会自动扫描所有文章，提取标题、日期、标签和摘要，按时间倒序生成文章列表，**不需要手动编辑 `index.md`**。
...
2. 文件名是否以 `.md` 结尾？
3. 文件名是否以 `YYYY-MM-DD-` 开头（这样才能正确排序）？
4. 第一行是否以 `# ` 开头写标题？
5. 第三行是否包含 `发布时间：YYYY-MM-DD`？
6. 查看 GitHub Actions 是否运行失败，如有失败需要看日志修复。

> 💡 现在文章列表是**自动生成**的，每次构建时会自动扫描所有文章，不需要手动编辑 `index.md`。
...
├── docs/
│   ├── posts/          # 所有文章放在这里
│   │   ├── 2026-03-11-restart-blog.md
│   │   └── hello-world.md
│   │   └── index.md    # 文章列表，**自动生成**，不要手动编辑
│   ├── guide/          # 指南文档放在这里
│   │   └── writing.md  # 你现在看的这篇
│   ├── .vitepress/
│   │   ├── theme/
